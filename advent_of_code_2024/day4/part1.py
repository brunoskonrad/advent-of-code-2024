...
def has_word_horizontal(puzzle: list[list[str]], x: int, y: int) -> bool:
  try:
    return puzzle[x][y + 1] == 'M' and puzzle[x][y + 2] == 'A' and puzzle[x][y + 3] == 'S'
  except Exception:
    return False
...
def has_word_vertical(puzzle: list[list[str]], x: int, y: int) -> bool:
  try:
    return puzzle[x + 1][y] == 'M' and puzzle[x + 2][y] == 'A' and puzzle[x + 3][y] == 'S'
  except Exception:
    return False
...
def has_word_diagonal_top_right(puzzle: list[list[str]], x: int, y: int) -> bool:
  try:
    return puzzle[x + 1][y + 1] == 'M' and puzzle[x + 2][y + 2] == 'A' and puzzle[x + 3][y + 3] == 'S'
  except Exception:
    return False
